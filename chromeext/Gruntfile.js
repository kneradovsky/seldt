module.exports = function(grunt) {
  ...
  var file, files = grunt.file.expand({cwd:'src/scripts'}, ['*.js']);
  for (var i = 0; i < files.length; i++) {
    file = files[i];
    fileMaps.browserify['build/unpacked-dev/scripts/' + file] = 'src/scripts/' + file;
    fileMaps.uglify['build/unpacked-prod/scripts/' + file] = 'build/unpacked-dev/scripts/' + file;
  }
  ...
  grunt.initConfig({
    ...
    mkdir: {
      ...
      js: { options: { create: ['build/unpacked-dev/scripts'] } }
    },
  ...
  grunt.registerTask('test', ['mochaTest','browserify']);
 ...
 grunt.registerTask('dev',['clean', 'test', 'mkdir:unpacked', 'copy:main', 'manifest',
   'mkdir:js', 'browserify', 'copy:prod', 'watch']);
